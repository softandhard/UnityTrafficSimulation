...
from models.feature_extractor_deepsort_CBAM import FeatureExtractor as FeatureExtractorDeepSortCBAM
...
def run_mtmc():
    ...
    # 加载特征提取模型
    scene_feat_ext_model = FeatureExtractorDeepSortCBAM(opt.scene_feat_ext_name, opt.scene_avg_type, opt.scene_feat_ext_weights)
    ...
    for fdx in range(0, np.max(f_nums) + 1):
        ...
        batch_scene_patch = torch.zeros((100 * len(cams), 3, opt.part_patch_size[0], opt.part_patch_size[1]), device='cuda').half()
        # Prepare images
        valid_cam = {}
        ...
        for pdx, d_pred in enumerate(det_pred):
            ...
            if len(d_pred) > 0:
                ...
                for *xyxy, conf, _ in reversed(d_pred):
                    ...
                    scene_patch = normalize(letterbox_part(scene_patch))
