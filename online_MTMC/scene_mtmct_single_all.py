...
def run_mtmc():
    ...
    print("res==", output_dir + 'single_all_mtmc_%s_%s.txt' % (opt.feat_ext_name, opt.avg_type))
    # Prepare others
    ...
    for fdx in range(0, np.max(f_nums) + 1):
        ...
        # pdist：返回一个精简的距离矩阵 Y 计算给定数据集中所有成对样本之间的距离。
        # 这个函数会返回一个关于距离的紧凑形式的距离矩阵，而不是一个完整的距离矩阵
