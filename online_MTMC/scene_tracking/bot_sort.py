...
from online_MTMC.scene_tracking.track import TrackState, BaseTrack, Track
    ...
    def update(self, cam, detections, features, scene_features):
        ...
        if len(boxes_first) > 0:
            detections_first = [Track(cam, cxcywh, s, f, scene_f)
                                for (cxcywh, s, f, scene_f) in zip(boxes_first, confidences_first, features_first, scene_features_first)]
        else:
            detections_first = []
        ...
        if opt.is_merge:
            ...
            if opt.merge_pattern == "average":
                ...
                for i in range(dists.shape[0]):
                    for j in range(dists.shape[1]):
                        dists[i, j] = (scene_dists[i, j] + dists[i, j]) / 2


        # 加权平均值
        # for i in range(scene_dists.shape[0]):
        #     for j in range(scene_dists.shape[1]):
        #         dists[i, j] = 0.8*scene_dists[i, j] + 0.2*dists[i, j]

        # dists[cos_dists > self.opt.cos_thr] = 1.
        dists[dists > self.opt.cos_thr] = 1.
        ...
        if len(boxes_second) > 0:
            detections_second = [Track(cam, x1y1x2y2, s, f, scene_f)
                                 for (x1y1x2y2, s, f, scene_f) in zip(boxes_second, confidences_second, features_second, scene_features_second)]
        else:
            detections_second = []
