...
class Track(BaseTrack):
    ...
    def __init__(self, cam, cxcywh, confidence, feat=None, scene_feat=None):
        ...
        self.update_features(feat, confidence)
        self.update_scene_features(scene_feat, confidence)
    ...
    def update_features(self, feat, confidence):
        if feat is not None:
            # Normalize
            ...
            if self.smooth_feat is None:
                ...
                # self.smooth_feat = confidence * self.smooth_feat + (1 - confidence) * feat
                self.smooth_feat = \
                    confidence / 2 * self.smooth_feat + confidence / 2 * self.smooth_scene_feat + (1 - confidence) * feat

            # Normalize
            self.smooth_feat /= np.linalg.norm(self.smooth_feat)

    def update_scene_features(self, feat, confidence):
        if feat is not None:
            # Normalize
            feat /= np.linalg.norm(feat)
            ...
            if self.smooth_scene_feat is None:
                ...
                # self.smooth_scene_feat = confidence * self.smooth_scene_feat + (1 - confidence) * feat
                self.smooth_scene_feat = \
                    confidence / 2 * self.smooth_scene_feat + confidence / 2 * self.smooth_feat + (1 - confidence) * feat
    ...
    def update(self, new_det, frame_id):
        ...
        if new_det.curr_feat is not None:
            self.update_features(new_det.curr_feat, new_det.confidence)
        if new_det.curr_scene_feat is not None:
            self.update_scene_features(new_det.curr_scene_feat, new_det.confidence)
