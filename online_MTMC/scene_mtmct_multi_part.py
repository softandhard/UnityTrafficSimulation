...
def run_mtmc():
    ...
    result_txt = open(output_dir + 'multi_part_mtmc_%s_%s.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
        ...
        for pdx, pred in enumerate(preds):
            ...
            if len(pred) > 0:
                ...
                for *xyxy, conf, _ in reversed(pred):
                    ...
                    # 获取目标框的部分图像
                    part_patch = batch_img_ori[pdx][:, max(y1, 0):min(y2 + 1, img_h), max(x1, 0):min(x2 + 1 - int((x2 - x1) / 2), img_w)]
