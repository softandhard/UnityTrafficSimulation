import numpy as np
from online_MTMC.opts import opt
from online_MTMC.scene_opts import opt as scene_opt
...
# Resize a rectangular image to a padded rectangular (128 x 128)
def letterbox_part(img, color=0):
    # shape = [height, width]
    shape = img.shape[1:]
    ratio = min(float(scene_opt.part_patch_size[0]) / shape[0], float(scene_opt.part_patch_size[1]) / shape[1])

    # new_shape = [channel, height, width]
    new_shape = (round(shape[0] * ratio), round(shape[1] * ratio))

    # Padding
    dh = (scene_opt.part_patch_size[0] - new_shape[0]) / 2
    dw = (scene_opt.part_patch_size[1] - new_shape[1]) / 2
