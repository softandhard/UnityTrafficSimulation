...
from online_MTMC.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from online_MTMC.utils.torch_utils import torch_distributed_zero_first
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        ...
        if any(videos):
            ...
            self.cap = None
        assert self.nf > 0, f'No images or videos found in {self.path}. ' \
    ...
    def __next__(self, cam, fdx):
        ...
        img_name = self.path[:-1] + '%07d.jpg' % fdx

        if os.path.isfile(img_name):
            img0 = cv2.imread(img_name)  # BGR
        else:
            return img_name, None, None, self.cap
        ...
        return img_name, img, img0, self.cap
...
class LoadImages_Ori:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        p = str(Path(path).absolute())  # os-agnostic absolute path
        if '*' in p:
...
class LoadStreams_Ori:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
