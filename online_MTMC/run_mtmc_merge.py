...
def run_mtmc():
    ...
    # 加载特征提取模型
    scene_feat_ext_model = FeatureExtractor(opt.scene_feat_ext_name, opt.scene_avg_type, opt.scene_feat_ext_weights)
    ...
    result_txt = open(output_dir + 'merge_mtmc_%s_%s.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
    print("res==", output_dir + 'merge_mtmc_%s_%s.txt' % (opt.feat_ext_name, opt.avg_type))
    # Prepare others
    ...
    for fdx in range(0, np.max(f_nums) + 1):
        ...
        batch_part_patch = torch.zeros((100 * len(cams), 3, opt.patch_size[0], opt.patch_size[1]), device='cuda').half()
        batch_img_cam = {}
        # Prepare images
        valid_cam = {}
        ...
        for pdx, pred in enumerate(preds):
            ...
            if len(pred) > 0:
                ...
                for *xyxy, conf, _ in reversed(pred):
                    ...
                    batch_part_patch[det_count] = torch.fliplr(patch) if cams[pdx] == 'c008' else patch
                    det_count += 1
