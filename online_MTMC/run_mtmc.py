...
from online_MTMC.utils.utils import letterbox, class_agnostic_nms, pairwise_tracks_dist
        ...
        for pdx, pred in enumerate(preds):
            ...
            if len(pred) > 0:
                ...
                for *xyxy, conf, _ in reversed(pred):
                    ...
                    # patch = batch_img_ori[pdx][:, max(y1, 0):min(y2 + 1, img_h), max(x1, 0):min(x2 + 1 -int((x2 - x1) / 2)
                    #                                                                             , img_w)]
