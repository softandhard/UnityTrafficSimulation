...
import random
import numpy as np
from opts_deepsort_CBAM import opt
from torchvision import transforms
from online_MTMC.utils.sklearn_dunn import dunn
from online_MTMC.tracking.bot_sort import BoTSORT
from online_MTMC.utils.datasets import LoadImages
from models.experimental import attempt_load
from scipy.spatial.distance import pdist, squareform
from scipy.cluster.hierarchy import linkage, fcluster
from models.feature_extractor_deepsort_CBAM import FeatureExtractor
...
def run_mtmc():
    ...
    output_dir_c006 = opt.output_dir + '%s/' % opt.det_name + 'c006/'
    output_dir_c007 = opt.output_dir + '%s/' % opt.det_name + 'c007/'
    output_dir_c008 = opt.output_dir + '%s/' % opt.det_name + 'c008/'
    output_dir_c009 = opt.output_dir + '%s/' % opt.det_name + 'c009/'

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    if not os.path.exists(output_dir_c006):
        os.makedirs(output_dir_c006)
    if not os.path.exists(output_dir_c007):
        os.makedirs(output_dir_c007)
    if not os.path.exists(output_dir_c008):
        os.makedirs(output_dir_c008)
    if not os.path.exists(output_dir_c009):
        os.makedirs(output_dir_c009)

    # Prepare result txt file
    result_txt = open(output_dir + 'deepsort_CBAM_mtmc_%s_%s.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
    result_txt_c006 = open(output_dir_c006 + 'deepsort_CBAM_mtmc_%s_%s_c006.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
    result_txt_c007 = open(output_dir_c007 + 'deepsort_CBAM_mtmc_%s_%s_c007.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
    result_txt_c008 = open(output_dir_c008 + 'deepsort_CBAM_mtmc_%s_%s_c008.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
    result_txt_c009 = open(output_dir_c009 + 'deepsort_CBAM_mtmc_%s_%s_c009.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
    ...
    for cam in cams:
        ...
        print("f_nums==", f_nums)
        # Prepare 2
        roi_masks[cam] = cv2.imread('C:/Users/cjh/Desktop/MTMCT/online_MTMC/preliminary/rois/%s.jpg' % cam, cv2.IMREAD_GRAYSCALE)
        overlap_regions[cam] = {}
        for cam_ in cams:
    ...
    with torch.autocast('cuda'):
        for _ in range(10):
            ...
            feat_ext_model(torch.rand((10, 3, opt.part_patch_size[0], opt.part_patch_size[1]), device='cuda').half())
    ...
    for fdx in range(0, np.max(f_nums) + 1):
        ...
        batch_patch = torch.zeros((100 * len(cams), 3, opt.part_patch_size[0], opt.part_patch_size[1]), device='cuda').half()
        batch_img_cam = {}
        # Prepare images
        valid_cam = {}
        ...
        for pdx, pred in enumerate(preds):
            ...
            if len(pred) > 0:
                ...
                for *xyxy, conf, _ in reversed(pred):
                    ...
                    patch = normalize(letterbox_part(patch))
        ...
        for cdx in range(num_cluster):
            ...
            # infos == [tdx, online_tracks[tdx].global_id]
            infos = []
        ...
        #     if not os.path.exists("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c006"):
        #         os.makedirs("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c006")
        #     if not os.path.exists("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c007"):
        #         os.makedirs("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c007")
        #     if not os.path.exists("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c008"):
        #         os.makedirs("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c008")
        #     if not os.path.exists("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c009"):
        #         os.makedirs("C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/img_result/c009")
        ...
        for track in online_tracks:
            ...
            if track.cam[-1] == "6":
                print('%d %d %d %d %d %d %d -1 -1' % (int(track.cam[-1]), track.global_id, temp_align[track.cam][fdx],
                                                  int(left), int(top), int(w), int(h)), file=result_txt_c006)
            if track.cam[-1] == "7":
                print('%d %d %d %d %d %d %d -1 -1' % (int(track.cam[-1]), track.global_id, temp_align[track.cam][fdx],
                                                  int(left), int(top), int(w), int(h)), file=result_txt_c007)

            if track.cam[-1] == "8":
                print('%d %d %d %d %d %d %d -1 -1' % (int(track.cam[-1]), track.global_id, temp_align[track.cam][fdx],
                                                  int(left), int(top), int(w), int(h)), file=result_txt_c008)

            if track.cam[-1] == "9":
                print('%d %d %d %d %d %d %d -1 -1' % (int(track.cam[-1]), track.global_id, temp_align[track.cam][fdx],
                                                  int(left), int(top), int(w), int(h)), file=result_txt_c009)
