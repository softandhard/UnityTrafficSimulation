...
from models.feature_extractor_resnext import FeatureExtractor as FeatureExtractorResnext
...
def run_mtmc():
    ...
    # 加载特征提取模型
    scene_feat_ext_model = FeatureExtractorResnext(opt.scene_feat_ext_name, opt.scene_avg_type, opt.scene_feat_ext_weights)
    ...
    output_dir = opt.output_dir + 'mask-rcnn/'
    ...
    for cam in cams:
        # Read
        det_dir = os.path.join(opt.data_dir, cam) + '/det/det_mask_rcnn.txt'
        det_results[cam] = np.loadtxt(det_dir, delimiter=',')[:, [0, 2, 3, 4, 5, 6]]

        # xywh to cxcywh
        det_results[cam][:, 1] += det_results[cam][:, 3] / 2
        det_results[cam][:, 2] += det_results[cam][:, 4] / 2
    ...
    for cam in cams:
        ...
        # Prepare 2
        roi_masks[cam] = cv2.imread('./preliminary/rois/%s.jpg' % cam, cv2.IMREAD_GRAYSCALE)
        overlap_regions[cam] = {}
        for cam_ in cams:
    ...
    with torch.autocast('cuda'):
        for _ in range(10):
            ...
            scene_feat_ext_model(torch.rand((10, 3, opt.patch_size[0], opt.patch_size[1]), device='cuda').half())
    ...
    for fdx in range(0, np.max(f_nums) + 1):
        ...
        batch_scene_patch = torch.zeros((100 * len(cams), 3, opt.patch_size[0], opt.patch_size[1]), device='cuda').half()
        # Prepare images
        valid_cam = {}
        ...
        det_result = []
        for cam in cams:
            det_result.append((det_results[cam][det_results[cam][:, 0] == temp_align[cam][fdx]])[:, 1:])

        det_pred = np.zeros((len(det_result), 100, 6))
        for ddx, d_result in enumerate(det_result):
            det_pred[ddx][:len(d_result), :5] = d_result

        det_pred = torch.tensor(det_pred).cuda().half()

        # NMS
        det_pred = non_max_suppression(det_pred, opt.conf_thres, opt.iou_thres, classes=None, agnostic=opt.agnostic_nms)
        ...
        for pdx, d_pred in enumerate(det_pred):
            ...
            if len(d_pred) > 0:
                ...
                for *xyxy, conf, _ in reversed(d_pred):
                    # Convert to integer
                    ...
                    # 获取目标框的部分图像
                    scene_patch = batch_img_ori[pdx][:, max(y1, 0):min(y2 + 1, img_h), max(x1, 0):min(x2 + 1, img_w)]
                    scene_patch = normalize(letterbox(scene_patch))

                    # 缩放为指定分辨率 patch [1, 3, 384, 384]
                    batch_patch[det_count] = torch.fliplr(patch) if cams[pdx] == 'c008' else patch
                    batch_scene_patch[det_count] = torch.fliplr(scene_patch) if cams[pdx] == 'c008' else scene_patch
                    det_count += 1
        ...
        # 提取背景特征
        with torch.autocast('cuda'):
            batch_scene_patch = batch_scene_patch[:det_count]
            batch_scene_feat = scene_feat_ext_model(batch_scene_patch)
        ...
        online_feats = np.array([track.get_scene_feature(mode=opt.get_feat_mode) for track in online_tracks])
        online_scene_feats = np.array([track.get_scene_feature(mode=opt.get_feat_mode) for track in online_tracks])

        # p_dists 数组中的每个值表示对应向量之间的距离，值越大表示这两个向量之间的距离越远
        p_dists = pdist(online_feats, metric='cosine')
        p_dists = np.clip(p_dists, 0, 1)
        scene_p_dists = pdist(online_scene_feats, metric='cosine')
        scene_p_dists = np.clip(scene_p_dists, 0, 1)


        if opt.cluster_merge_pattern == "minimal":
            # 选择特征向量距离较近的特征值, 即数值较小的特征向量
            for i in range(len(p_dists)):
                if scene_p_dists[i] < p_dists[i]:
                    p_dists[i] = scene_p_dists[i]

        if opt.cluster_merge_pattern == "average":
            # 选择特征向量平均值
            for i in range(len(p_dists)):
                p_dists[i] = (scene_p_dists[i] + p_dists[i]) / 2
