...
import random
import numpy as np
from scene_opts import opt
from torchvision import transforms
from online_MTMC.utils.sklearn_dunn import dunn
from online_MTMC.scene_tracking.bot_sort import BoTSORT
from models.feature_extractor import FeatureExtractor as FeatureExtractorResnet
from models.feature_extractor_deepsort_CBAM import FeatureExtractor as FeatureExtractorDeepsort
...
def run_mtmc():
    ...
    feat_ext_model = FeatureExtractorResnet(opt.feat_ext_name, opt.avg_type, opt.feat_ext_weights)
    feat_ext_model = feat_ext_model.cuda().eval().half()

    # 加载特征提取模型
    scene_feat_ext_model = FeatureExtractorDeepsort(opt.scene_feat_ext_name, opt.scene_avg_type, opt.scene_feat_ext_weights)
    ...
    result_txt = open(output_dir + 'single_part_mtmc_%s_%s.txt' % (opt.feat_ext_name, opt.avg_type), 'w')
