...
def get_args():
    ...
    parser.add_argument("--data", nargs=2, default=['C:/Users/cjh/Desktop/MTMCT/eval/gt/ground_truth_test.txt',
                                            'C:/Users/cjh/Desktop/MTMCT/online_MTMC/outputs/yolov7-w6/merge_mtmc_resnext101_ibn_a_gap.txt'],
    ...
    parser.add_argument('-ds', '--dstype', type=str, default='validation',
...
def eval(test, pred, **kwargs):
    ...
    def removeOutliersROI(df, dstype='train', roidir='ROIs', cid=None):
        ...
        roi = loadroi(cid)
        height, width = roi.shape
        # Loop over objects and check for outli         ers
        for i, row in df.iterrows():
            if isROIOutlier(row, roi, height, width):
                df.at[i, 'NotOutlier'] = False
