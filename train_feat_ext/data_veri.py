...
class TrainDataManager(Dataset):
    def __init__(self):
        ...
        # config_veri.num_ide_class == 数据集类别数
        for ndx in range(config_veri.num_ide_class):
            self.img_path_obj_id[ndx] = []

        # Read image names
        self.img_names = os.listdir(config_veri.tr_data_dir)
        # print("img-name==", self.img_names)
        # Read image
        for img_name in self.img_names:
            obj_id = int(img_name.split('_')[0])
            print("obj-id", obj_id)
            self.img_path_obj_id[obj_id].append([cv2.imread(config_veri.tr_data_dir + img_name)[:, :, [2, 1, 0]], obj_id])

        # min_num = len(self.img_path_obj_id[0])
        # print("all-len==", len(self.img_path_obj_id))
        # for i in range(len(self.img_path_obj_id)):
        #     if int(len(self.img_path_obj_id[i])) <= min_num:
        #         min_num = len(self.img_path_obj_id[i])
        # # veri 数据集中每个类别最小样本数为 11
        # print("min==", min_num)
        # Logging
    ...
    def gen_left(self):
        ...
        for obj_id in self.img_path_obj_id:
            if len(self.img_path_obj_id[obj_id]) > config_veri.num_samples_per_id:
                left[obj_id] = list(range(len(self.img_path_obj_id[obj_id])))
        return left

    # Generate queue - c samples for each object ID
    # 每个对象ID的样本 == 每个对象ID的采样数
    def gen_new_img_path_obj_id(self):
        ...
        for obj_id in self.img_path_obj_id.keys():
            ...
            # Fill insufficient queue
            elif len(self.img_path_obj_id[obj_id]) < config_veri.num_samples_per_id:
                ...
                while len(new_img_path_obj_id[obj_id]) < config_veri.num_samples_per_id:
            ...
            # Pick carefully
                ...
                if len(self.left[obj_id]) < config_veri.num_samples_per_id:
                    queue += copy.deepcopy(self.left[obj_id])
                    self.left[obj_id] = list(range(len(self.img_path_obj_id[obj_id])))

                # Pick
                temp_queue = random.sample(self.left[obj_id], config_veri.num_samples_per_id - len(queue))
                queue += temp_queue
    ...
    def gen_pk_batch_queue(self):
        pk_batch_queue = []
        while len(self.possible_obj_id) >= config_veri.p_num:
            # Sample p queue
            p_queue = random.sample(self.possible_obj_id, config_veri.p_num)

            # Construct pk batch queue
            for obj_id in p_queue:
                for _ in range(config_veri.k_num):
                    pk_batch_queue.append(self.new_img_path_obj_id[obj_id].pop())

                if len(self.new_img_path_obj_id[obj_id]) < config_veri.k_num:
    ...
    def initiate(self):
        # Ready
        self.possible_obj_id = list(range(config_veri.num_ide_class))
